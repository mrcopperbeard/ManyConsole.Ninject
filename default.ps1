...
task GenerateAssemblyInfo {
	...
	foreach($projectFile in $projectFiles) {
		...
		Generate-Assembly-Info `
			...
			-company "n/a" `
			-product "ManyConsole.Ninject $version" `
			-version "$version" `
			-fileversion "$version" `
...
task Build -depends Cleanup,GenerateAssemblyInfo {
    ...
    exec { &"C:\Windows\Microsoft.NET\Framework\$v4_net_version\MSBuild.exe" ManyConsole.Ninject.sln /T:"Clean,Build" /property:OutDir="$buildDirectory\" }    
}

task RunTests {
    exec { & "$baseDirectory\packages\NUnit.Runners.2.6.1\tools\nunit-console.exe" "$buildDirectory\ManyConsole.Ninject.Tests.dll" -xml:"$buildDirectory\TestResults.xml" }
}
...
task BuildNuget -depends Build {
    ...
    cp "$buildDirectory\ManyConsole.Ninject.dll" "$nugetTarget\lib\"
    cp "$buildDirectory\ManyConsole.Ninject.pdb" "$nugetTarget\lib\"

    $old = pwd
    cd $nugetTarget

    ..\..\tools\nuget.exe spec -a ".\lib\ManyConsole.Ninject.dll"

    update-xml "ManyConsole.Ninject.nuspec" {
        ...
        for-xml "//package/metadata" {
            ...
            set-xml -exactlyOnce "//licenseUrl" "https://github.com/fschwiet/ManyConsole.Ninject/blob/master/LICENSE.txt"
            set-xml -exactlyOnce "//projectUrl" "https://github.com/fschwiet/ManyConsole.Ninject/"
    ...
    ..\..\tools\nuget pack "ManyConsole.Ninject.nuspec"
